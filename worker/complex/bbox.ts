import {
  COMPLEX_BBOX_ITEM_LIMIT,
  type ComplexBboxResponse,
  type ComplexSummary,
} from '../../shared/complex'

const BAD_REQUEST_STATUS = 400
const RESULT_QUERY_LIMIT = COMPLEX_BBOX_ITEM_LIMIT + 1
...
export const queryComplexes = async (
  ...
  const truncated = result.results.length > COMPLEX_BBOX_ITEM_LIMIT
  return {
    items: result.results.slice(0, COMPLEX_BBOX_ITEM_LIMIT).map(toSummary),
    truncated,
  }
}
